...
export default function BlogDetailsHome({ blogData }: { blogData: Blog }) {
    ...
    async function handleCommentSave() {
        ...
        const responce = await fetch(`/api/blog-post/add-comment`, {
            method: 'PUT',
            headers: {
                ...
                <div className="-mx-4 flex flex-col gap-4 items-center justify-center">
                    <div className="w-full px-4 lg:w-8/12">
                        <div>
                            ...
                            <div className="mb-10 flex flex-wrap items-center justify-between border-b border-body-color border-opacity-10 pb-4 dark:border-white dark:border-opacity-10">
                                ...
                                <div className="mb-5">
                                    <Link className="inline-flex items-center justify-center rounded-full bg-primary py-2 px-4 text-sm font-semibold text-white" href={`/category/${blogData?.category}`}>
                                        {blogData?.category}
                                    </Link>
                                </div>
                            </div>
                            <div>
                                <div className="mb-10 w-full overflow-hidden rounded">
                                    <div className="relative aspect-[97/60] w-full sm:aspect-[20/12]">
                                        <Image className="object-cover object-center" src={blogData?.image || ''} alt="Blog" fill />
                                    </div>
                                </div>
                    ...
                    <section className="dark:bg-gray-900 py-8 lg:py-16 w-full lg:w-8/12">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-lg lg:text-2xl font-bold text-black dark:text-white">Discussion ({blogData?.comments?.length})</h2>
                        </div>
                        {
